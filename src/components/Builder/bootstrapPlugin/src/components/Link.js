    ...
    comps.addType('link', {
        model: textModel.extend({
            ...
            init2() {
                ...
                this.listenTo(this, 'change:attributes:actions',this.handleChangeActions) // for when href changes
            },
            handleChangeActions(a,b,options={}){
                this.setAttributes({href:'#'+this.getAttributes().actions});
            },
            setupToggle(a, b, options = {}) { // TODO this should be in the dropdown comp and not the link comp
                ...
                if (href && href.length > 0 && !href.match(/^#/)) {
                    ...
                    const els = this.em.get('Editor').DomComponents.getWrapper().find(href);console.log(els)
                    if (els.length > 0) {
                ...
                // this.set('attributes', attrs, {ignore: true});/** we are using action dropdown for default links */
            },
            // classesChanged(e) {
            //     console.log('classes changed');
            //     if (this.attributes.type === 'link') {    /**  we are not using btn class inside a tag */
